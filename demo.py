"""Demo script to showcase the LangChain application features."""

import os
from dotenv import load_dotenv
load_dotenv() # New
from langchain_google_genai import ChatGoogleGenerativeAI
from mock_tools import FakeWeatherSearchTool, FakeCalculatorTool, FakeNewsSearchTool
from router import ConversationRouter

def run_mock_demo(): #New
    """Fallback demo when no API key is set (uses mock responses)."""
    print("⚠️  No Google API key found. Using mock responses for demo.")
    demo_queries = [
        "What's the weather like in Tokyo?",
        "Calculate 5 * 3",
        "Find me news about machine learning",
        "Hello! How are you doing today?"
    ]
    for i, q in enumerate(demo_queries, 1):
        print(f"\n--- Demo {i} (MOCK) ---")
        print(f"Query: {q}")
        ql = q.lower()
        if "weather" in ql:
            print("Response: [MOCK] It is sunny and warm in Tokyo.")
        elif "calculate" in ql:
            print("Response: [MOCK] 5 * 3 = 15.")
        elif "news" in ql:
            print("Response: [MOCK] Here are 3 recent ML headlines...")
        else:
            print("Response: [MOCK] Hi! I’m running in mock mode.")


def run_demo():
    """Run a demonstration of the application features."""
    print("🚀 LangChain Application Demo")
    print("=" * 40)

    # Load environment variables
    # os.environ[]

    # Check if API key is available
    google_api_key=os.getenv("GOOGLE_API_KEY")
    if not os.getenv("GOOGLE_API_KEY"):
        print("⚠️  No Google API key found. Using mock responses for demo.")
        run_mock_demo()
        return

    # Initialize LLM
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.5-flash",
        temperature=0.7,
        google_api_key=os.getenv("GOOGLE_API_KEY")
    )

    # Initialize tools (all features) #New
    tools = [FakeWeatherSearchTool(), FakeCalculatorTool(), FakeNewsSearchTool()]

    # Initialize router #New
    router = ConversationRouter(llm, tools=tools)

    # Demo queries
    demo_queries = [
        "What's the weather like in Tokyo?",
        "Calculate 5 * 3",
        "Find me news about machine learning",
        "Hello! How are you doing today?"
    ]

    print("\n🎯 Running demo queries...")

    for i, query in enumerate(demo_queries, 1):
        print(f"\n--- Demo {i} ---")
        print(f"Query: {query}")
        try:
            response = router.process_message(query)
            print(f"Response: {response}")
        except Exception as e:
            print(f"Error: {str(e)}")

if __name__ == "__main__":
    run_demo()